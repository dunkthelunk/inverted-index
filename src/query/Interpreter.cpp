  ...
  switch (Node->op().type()) {
    case TokenType::AND :
      std::set_intersection(LeftResult.begin(), LeftResult.end(),
          RightResult.begin(), RightResult.end(), std::inserter(Node->result(), Node->result().end()));
      break;
    case TokenType::OR :
...
std::set<TermRecord, TermRecordCompare> Interpreter::compute(BinaryOpASTNode *Node){
  return {};
} 
std::set<TermRecord, TermRecordCompare> Interpreter::compute(WordASTNode *Node){
  return {};
}
