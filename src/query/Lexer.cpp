...
bool Lexer::isSpecialChar(char C) {
  ...
  return std::any_of(SpecialChars.begin(), SpecialChars.end(), [C](char ArrC) {
      return C == ArrC;
      });
}

bool Lexer::isCurrentSpecial() {
  return isSpecialChar(CurrentChar);
}

bool Lexer::isPrevSpecial() {
  return isSpecialChar(TokensOnlyText.back());
}
